...
def load_data_set(vocab_size, dataset_type, level='l3'):
    ...
    if os.path.exists("./data/%s.x_train.p"%dataset_type):
        ...
        pdb.set_trace()
        x_test = dataLoader.read(data_loc=data_loc, file_name="wos_data_n_test.csv", column='l2')
    ...
    if dataset_type == "DBpedia":
        data_loc = '/home/ml/ksinha4/mlp/hier-class/data'
        x_train = dataLoader.read(data_loc=data_loc, file_name="df_small_train.csv", column=level)
        x_test = dataLoader.read(data_loc=data_loc, file_name="df_small_test.csv",  column=level)
    elif dataset_type == "WIKI":
        data_loc = '/home/ml/ksinha4/datasets/data_WIKI'
        x_train = dataLoader.read(data_loc=data_loc, file_name="full_docs_2_train.csv", column=level)
        x_test = dataLoader.read(data_loc=data_loc, file_name="full_docs_2_test.csv",  column=level)
        # "/home/ml/ksinha4/datasets/data_WOS/WebOfScience/WOS46985"
    elif dataset_type == "WOS":
        data_loc = '/home/ml/ksinha4/mlp/hier-class/data'
        x_train = dataLoader.read(data_loc=data_loc, file_name="wos_data_n_train.csv", column=level)
        x_test = dataLoader.read(data_loc=data_loc, file_name="wos_data_n_test.csv", column=level)
    else:
