...
def load_data_set(type,max_len,vocab_size,batch_size):
    ...
    print("----initial data loading----")
    dataLoader = Data_Utility()
    x_train_texts, y_train = dataLoader.read(data_loc=data_loc, file_name="df_small_train.csv")
    x_val_texts, y_val = x_train_texts[:int(0.1*len(y_train))], y_train[:int(0.1*len(y_train))]
    x_train_texts, y_train = x_train_texts[int(0.1 * len(y_train)):], y_train[int(0.1 * len(y_train)):]
    x_test_texts, y_test = dataLoader.read(data_loc=data_loc, file_name="df_small_test.csv")
    print("----finish data loading----{%d} train,{%d} val, {%d} test"
          %(len(y_train),len(y_val),len(y_test)))


    w2i, i2w = dataLoader.assign_word_ids(x_train_texts+x_val_texts+x_test_texts, vocab_size=vocab_size)
    dataLoader.assign_category_ids(list(y_train) + list(y_val) + list(y_test))
    print("----processed {%d} word_2_id, {%d}cat_2_id----"%(len(w2i),len(dataLoader.cat2id)))
