...
class Data_Utility(data.Dataset):
    ...
    def read(self,  data_loc='',
            file_name='full_docs_2.csv', tokenization='word', column='l3'):
        """
        ...
        # create dictionary
        assert len(df_texts) == len(df[column]) # column = l3 is the end level label
        # print("finished tokenizing %d data instances"%len(df['l3']))

        df = pandas.DataFrame(list(zip(df_texts, list(df[column]))))
        df.columns=['text', 'label']
        # return df_texts, df['l3']
        return df

    def WOSread(self, data_loc='',
            file_name='X.txt', tokenization='word', column='l3'):
        df = pandas.read_csv(data_loc + '/' + file_name)
        df = df.sample(frac=1).reset_index(drop=True)
        # print(df.head(10))
        # print("finished loading %d data instances" % len(df))
        ...
        # df_texts = df.text.apply(self.tokenize)
        # create dictionary
        assert len(df_texts) == len(df[column])  # l3 is the end level label
        # print("finished tokenizing %d data instances"%len(df['l3']))
