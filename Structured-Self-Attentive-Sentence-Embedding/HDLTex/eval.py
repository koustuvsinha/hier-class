...
def evaluate(l1_model,l2_models,L2_dict_i2y,x_test,y_test):
    ...
    for i in range(len(y_test)):
        ...
        # print(l1_pred,l1_true,l2_pred,l2_true)
        if i%500==0:
            print('eval processed',i)
    l1_correct = sum([l1_y_pred[i]==l1_y_true[i] for i in range(len(l1_y_pred))])
    l2_correct = sum([l2_y_pred[i] == l2_y_true[i] for i in range(len(l2_y_pred))])
    l1_acc = accuracy_score(l1_y_true, l1_y_pred)
    l2_acc = accuracy_score(l2_y_true, l2_y_pred)
    print("l1 acc: %.4f,  l2 acc: %.4f ,l1_correct: %d, l2_correct:%d, total:%d"
          %(l1_acc,l2_acc,l1_correct,l2_correct,len(l1_y_pred)))
