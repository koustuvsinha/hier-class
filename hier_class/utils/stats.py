...
class Statistics():
    ...
    """
    def __init__(self, batch_size=0, max_levels=3, exp_name='', data=None):
        self.epoch = -1
        self.step = 0
        self.train_loss = []
    ...
    def update_validation(self, validation_loss, validation_accuracy, attn=None, src=None, preds=None):
        ...
        flat_attns = []
        for level_attn_var in attn:
            comb_attn = level_attn_var.data.cpu().numpy()
            # separate the attentions by layer
            flat_attn = comb_attn.reshape(self.batch_size, -1, comb_attn.shape[2])
